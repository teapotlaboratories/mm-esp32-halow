...
/* The active key's TX sequence number (PN) for `key_id` — the host-managed CCMP PN source for the
 * mesh SW-crypto TX path (mac80211 increments then uses, so callers increment_tx_seq before reading).
 * NOTE: takes a resolved key_id (connection_keys no longer resolves key_type internally at 2.11.2). */
